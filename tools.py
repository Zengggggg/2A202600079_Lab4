...
def search_flights(origin: str, destination: str) -> str:
    ...
    # - Gợi ý: format giá tiền có dấu chấm phân cách (1.450.000d)
    ...
    result = []
    routes = f"{origin} → {destination}" if not reversed_flag else f"{destination} → {origin} (đảo chiều)"
    result.append(f"Chuyến bay từ {routes}:\n")
    for f in flights:
        result.append(
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    for h in filtered:
        result.append(
            ...
            f"Rating: {h['rating']} | {format_price(h['price_per_night'])}/đêm"
        )
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        return (
            "Format expenses không hợp lệ.\n"
            "Ví dụ đúng: 've_may_bay:890000,khach_san:650000'"
        )
    ...
    for name, value in expense_dict.items():
        ...
        result.append(f"- {pretty_name}: {format_price(value)}")
    ...
    if remaining >= 0:
        ...
        result.append(
            f"ngân sách {format_price(abs(remaining))}! Cần điều chỉnh."
        )
